...
@lru_cache(maxsize=1)
def fr5_ag95_c_curobo_cfg() -> dict:
    assets_dir = Path(__file__).parent / "assets"
    # Note: use ur5e_robotiq_ag95_c.yml for UR5e with camera mount (on MIT setup)
    cfg = load_yaml(str(assets_dir / "fr5_ag95_c.yml"))
...
def _fr5_ag95_c_cfg_dict() -> dict:
    return fr5_ag95_c_curobo_cfg()["robot_cfg"]


def get_fr5_ag95_c_kinematics_model() -> CudaRobotModel:
    """cuRobo robot kinematics model."""
    robot_cfg = RobotConfig.from_dict(_fr5_ag95_c_cfg_dict())
    kinematics_model = CudaRobotModel(robot_cfg.kinematics)
    return kinematics_model


def get_fr5_ag95_c_ik_solver(
    ...
    """
    robot_cfg = _fr5_ag95_c_cfg_dict()
...
def get_fr5_ag95_c_gripper_spheres(tensor_args: TensorDeviceType = TensorDeviceType()) -> Float[torch.Tensor, "num_spheres 4"]:
...
def load_fr5_ag95_c_rerun(load_mesh: bool = True) -> RerunRobot:
    robot_cfg = _fr5_ag95_c_cfg_dict()
    ...
    return RerunRobot("fr5_ag95_c", urdf, q_neutral=fr5_home, load_mesh=load_mesh)
